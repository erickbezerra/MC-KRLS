...
for ze = 1:NVarM
        ...
        for zedd = 1:NVarembedding
		    ...
		    %X = [ X1 X2 X3 X4 X5 X6 X7 X8 ]; Y = [ Y1 Y2 Y3 Y4 Y5 Y6 Y7 Y8 ]; % ( V T T T T )		    
                    ...
                    for i=1:N,
                        ...
                        if (size(kaf1.dict,1)>0 && size(kaf2.dict,1)>0 )
                            Y_est(i,:)=(Y_est1(i,:)+Y_est2(i,:))/2;
                        end
                        if (size(kaf1.dict,1)>0 && size(kaf2.dict,1)==0) || (size(kaf1.dict,1)==0 && size(kaf2.dict,1)>0)
                            Y_est(i,:)=(Y_est1(i,:)+Y_est2(i,:));
                        end
                        if (size(kaf1.dict,1)==0 && size(kaf2.dict,1)==0 )
                        ...
                        if size(kaf1.dict,1) >= kaf1.M/4 % if dic size is above 25% of the dictionary size (VarM) starts the second kernel machine.
                            kaf2 = kaf2.train(X(i,:),Y(i,:)); % train with one input-output pair
                        end
                        
                        % Count KafMAX.M iterations as a criteria to start matrix delet
                        % process
                        if (size(kaf1.dict,1)==kaf1.M && size(kaf2.dict,1)==kaf2.M )
                        ...
                        if deletcounter == deletcountermax % (i + size(kaf1.dict,1)+size(kaf2.dict,1)+size(kaf3.dict,1) == i + kaf1.M+kaf2.M+kaf3.M*2 )
                            ...
                            MSEtemp = [ sum(mean(SE1,2)) sum(mean(SE2,2))  ];
    ...
    Exec_time = toc;
    % Saving file  
    outfile = ['025n_M2KRLS_T_M',int2str(VarM(ze)),'_1'];
