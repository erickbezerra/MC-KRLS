...
classdef krlst
    ...
    properties (GetAccess = 'public', SetAccess = 'public')
        ...
        NY = 1; % for kernel machines with x > 1 column
        Age = 0; % Age
    end
    ...
    methods
        ...
        function [mean_test, var_test] = evaluate(kaf,x) % evaluate
            if size(kaf.dict,1)>0
                ...
                mean_test = zeros(size(x,1),kaf.NY); % prior
                % var_test % prior
            end
        end
        ...
        function kaf = train(kaf,x,y) % train the algorithm
            ...
            if m<1 % initialize
                ...
                if kaf.NY > 1 % In case the output has more than 1 column
                    sy2ptemp = []; diftemp = (y-y_mean);
                    for i = 1:kaf.NY
                        sy2ptemp = [ sy2ptemp diftemp(i)/sy2*p ];
                    end
                    kaf.mu = [kaf.mu; y_mean] + sy2ptemp; % a posteriori mean
                else
                    kaf.mu = [kaf.mu; y_mean] + (y - y_mean)/sy2*p; % a posteriori mean
                end
                % kaf.mu = [kaf.mu; y_mean] + (y - y_mean)/sy2*p; % posterior mean
                ...
                if (m>kaf.M  || gamma2<kaf.jitter)
                    if gamma2<kaf.jitter, % to avoid roundoff error
                        ...
                        if kaf.NY > 1 % In case the output has more than 1 column
                            diagkafkinv = [];
                            for i = 1:kaf.NY
                                diagkafkinv = [ diagkafkinv diag(kaf.Q) ];
                            end
                            criterion = abs((kaf.Q*kaf.mu)./diagkafkinv);
                        else
                            criterion = abs((kaf.Q*kaf.mu)./diag(kaf.Q));
                        end
                    end
                    % [~, r] = min(criterion); % remove element r, which incurs in the minimum error
                    [~,r] = min(sum(criterion,2));
                    r = r(1);
